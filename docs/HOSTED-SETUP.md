...
Physical iPhone capture, Wi-Fi joining, thermal behavior and recovery after a network interruption still need a real handset run. The camera release points at the hosted HTTPS API and owner site. TestFlight processing is separate from physical capture verification. This pilot has no automatic face blurring, automatic social publishing, team management or billing implementation. Review clips before sharing.
