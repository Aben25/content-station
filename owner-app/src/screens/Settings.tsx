...
export function Settings() {
  ...
  const rows: [string, string, (() => void)?][] = [
    ...
    ['Connected accounts', 'Publish approved clips to Facebook or Instagram.', () => navigate(R.accounts)],
    ...
    <div style={st(S_SCROLL)}>
      ...
      <div style={st('padding:8px 20px 24px;display:flex;flex-direction:column;gap:16px')}>
        <div style={st('background:#fff;border:1px solid rgba(23,22,20,.08);border-radius:20px;overflow:hidden;display:flex;flex-direction:column')}>
          {rows.map(([name, sub, open]) => {
            const Row = open ? 'button' : 'div';
            return (
              <Row key={name} onClick={open} style={{ ...st('display:flex;justify-content:space-between;align-items:center;padding:16px 18px;border:0;border-bottom:1px solid rgba(23,22,20,.08);gap:12px;background:none;text-align:left;width:100%;color:#171614'), cursor: open ? 'pointer' : 'default' }}>
        ...
        <div style={st('font-size:12px;color:#B8B3AA;padding:0 4px')}>Only Connected accounts can be changed here.</div>
